...
export default class GameoverEngine {
    ...
    start(){
        ...
        switch(State.playCharacter){
            case "rin":
                State.gameStage.addChild(State.object.image.BUTTON_TWITTER_GAMEOVER_RIN);
                break;
        }
        this.tick.add(()=>{
    ...
    handleLinkButtonEventListener(){
        ...
        return {
            add: ()=> {
                ...
                State.object.image.BUTTON_TWITTER_GAMEOVER_RIN.addEventListener('mousedown', tweet);
            },
            remove: ()=> {
                State.object.image.BUTTON_BACK_MENU_FROM_GAME.removeEventListener('mousedown', goToMenue);
                State.object.image.BUTTON_RESTART.removeEventListener('mousedown', restart);
                State.object.image.BUTTON_TWITTER_GAMEOVER_RIN.removeEventListener('mousedown', tweet);
            }
        }
    }
