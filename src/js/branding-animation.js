...
export default class BrandingAnimation{
    ...
    init(resolvePromise){
            ...
            if(target.text == '房'){
                this._timeline.addTween(
                    createjs.Tween.get(target, {loop:false})
                        ...
                        .wait((TIMELINE.EXPANTION + TIMELINE.SHRINK) * (BRAND_CHARACTERS.length - i - 1))
                        .wait(TIMELINE.CHARACTER_WAITING)
                        .to({x: this._x + this._width * position, scaleX:1}, TIMELINE.CHARACTER_EXTEND)
        ...
        this._timeline.addTween(
            createjs.Tween.get(hammer)
                .wait((TIMELINE.EXPANTION + TIMELINE.SHRINK) * BRAND_CHARACTERS.length)
                .to({y: this._y - this._height * 0.18}, TIMELINE.HAMMER_MOVING * 0.5, createjs.Ease.quadOut)
                .to({y: this._y - this._height * 0.05}, TIMELINE.HAMMER_MOVING * 0.5, createjs.Ease.quadIn));
